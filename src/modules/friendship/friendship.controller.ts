...
export const respondFriendship = async (req: Request, res: Response) => {
  try {
    ...
    const userId = req.user?.id; // ID del usuario autenticado por JWT

    const updated = await friendshipService.respondRequest(requestId, userId, status);
    ...
    if (error.message === 'FORBIDDEN') {
      return res.status(403).json({ message: 'No eres el destinatario de esta solicitud' });
    }
